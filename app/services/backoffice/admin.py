...
from app.schemas.backoffice.admin import (
    AdminCreate,
    AdminResponse,
    AdminDetailResponse,
    )
...
class AdminService:
    ...
    # 封装单个管理员
    @staticmethod
    async def get_admin_detail(db: AsyncSession, admin_id: int) -> Optional[AdminDetailResponse]:
        """Get admin details"""
        admin_query = select(Admin).where(Admin.id == admin_id)
        result = await db.execute(admin_query)
        admin = result.scalar_one_or_none()
        ...
        return AdminDetailResponse.model_validate(admin)
