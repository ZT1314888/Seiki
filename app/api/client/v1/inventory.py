...
from app.schemas.client.inventory import FaceCreateRequest, FaceUpdateRequest, BillboardType, FaceStatus
...
async def create_inventory(
    ...
    """Create an inventory face entry."""
...
async def delete_inventory(
    ...
    """Delete an inventory face entry by id."""
...
async def get_inventory_detail(
    ...
    """Retrieve full details for a single inventory face."""

    face = await client_inventory_service.get_face(db, face_id)
    return ApiResponse.success(
...
async def list_inventory(
    ...
    billboard_type: BillboardType | None = None,
    ...
    data = await client_inventory_service.list_faces(
        ...
        face_id=face_id,
        billboard_type=billboard_type.value if billboard_type else None,
...
async def update_inventory(
    ...
    """Edit inventory face while keeping immutable fields locked."""
...
@router.get("/tree")
async def get_inventory_tree(db: AsyncSession = Depends(get_db)):
    """Return hierarchical inventory tree grouped by media owner and network."""

    tree = await client_inventory_service.get_inventory_tree(db)
    return ApiResponse.success(
